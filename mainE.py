...
def main():
    ...
    checkpoint = torch.load('./checkpoint/EfficientFace_Trained_on_CAERS.pth.tar')#torch.load('./checkpoint/[07-14]-[15-24]-model_best.pth.tar')#
    ...
    if args.evaluate:
        validate(val_loader, model_cla, criterion, args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_acc, train_los = train(train_loader, model_cla, criterion, optimizer, epoch, args)

        # evaluate on validation set
        val_acc, val_los = validate(val_loader, model_cla, criterion, args)
        ...
        save_checkpoint(epoch, model_cla, optimizer,is_best, args)
...
def save_checkpoint(epoch, model, optimizer,is_best, args):
    state = {
        'epoch': epoch,
